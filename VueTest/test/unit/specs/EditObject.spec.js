...
it('все селекты кроме области не доступны в начале', () = > {
  expect(wrapper.vm.regionSelected;
).
toBeFalsy();
...
it('выбран регион', () = > {
  wrapper.setData({currentRegion: {id: '000001'}});
expect(wrapper.vm.regionSelected).toBeTruthy();
})
it('выбран город', () = > {
  wrapper.setData({currentCity: {id: '000001'}});
expect(wrapper.vm.citySelected).toBeTruthy();
})
it('выбрана улица', () = > {
  wrapper.setData({currentStreet: {id: '000001'}});
expect(wrapper.vm.streetSelected).toBeTruthy();
})
it('выбрано здание', () = > {
  wrapper.setData({currentBuilding: {id: '000001'}});
